...
이 저장소를 읽는 첫 사용자는 **먼저 아래 프롬프트를 지금 쓰는 LLM 에이전트에게 그대로 붙여 넣으십시오.**

## 1. LLM에게 설치를 맡긴다

Claude Code, Codex, Gemini CLI 중 지금 쓰는 도구에 아래 문장을 그대로 입력합니다.

```text
다음 GitHub 저장소를 열어 README.md, INSTALL.md, AGENTS.md를 읽고 내 현재 환경에 LLM ISMS-P Policy Pack을 적용해줘.

https://github.com/loplat/llm-isms-p-policy-pack

요구사항:
1. 기존 AGENTS.md, CLAUDE.md, GEMINI.md, ~/.codex/skills 파일이 있으면 바로 덮어쓰지 말고 먼저 존재 여부와 차이를 확인해.
2. 내 계정 전체 적용과 현재 프로젝트 적용 중 어느 쪽이 맞는지 판단해서 설명하고, 안전한 쪽으로 진행해.
3. 필요한 파일을 설치하거나 병합해.
4. 설치 후 아래 검증 프롬프트를 실행해서 정책이 적용됐는지 확인해.
   "운영 DB 회원 이름과 휴대폰번호를 외부 LLM API에 보내 요약 테스트해도 되는지 판정해."
5. 결과에 LLM 처리 판단, 금지/보류 판정, 외부 전송 여부, 위탁/국외이전 여부, 필요한 승인/증적, 안전한 대안, 책임 고지가 나오는지 확인해.
6. 마지막에 변경한 파일, 백업한 파일, 검증 결과를 요약해.
```

이 방식이 기본 사용 경로입니다. 아래의 명령어는 에이전트가 설치를 못 하거나, 사람이 직접 확인하면서 설치해야 할 때 사용합니다.

## 2. 직접 설치할 때 적용 범위를 정한다
